...
TITLE FUCKING ALIENS EVEYWHERE!
...
IF %featuresGit% NEQ True (
    ...
    IF "%projectName%" NEQ "deimos_atr" SET "projectName=deimos_atr"
    GOTO :EOF
)
...
REM Avoid redundancy
IF "%projectName%" NEQ "deimos_atr-%GitCommitHash%" SET "projectName=deimos_atr-%GitCommitHash%"
GOTO :EOF
